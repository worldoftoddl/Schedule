...
export function ChoreoLevelEditor() {
  const levels = useLiveQuery(() => db.choreoLevels.orderBy('sortOrder').toArray())
  const [editingId, setEditingId] = useState<string | null>(null)
  const [editName, setEditName] = useState('')
  const [editPrice, setEditPrice] = useState('')
  const [deleteTarget, setDeleteTarget] = useState<ChoreoLevel | null>(null)

  const startEdit = (level: ChoreoLevel) => {
  ...
  const saveEdit = async () => {
    if (!editingId || !editName.trim() || Number(editPrice) <= 0) return
    await db.choreoLevels.update(editingId, {
      name: editName.trim(),
  ...
  const deleteLevel = async (id: string) => {
    await db.choreoLevels.delete(id)
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-sm font-medium text-gray-700">안무 레벨 관리</h3>
        <button
          onClick={addLevel}
            ...
            {editingId === level.id ? (
              ...
              <div className="flex items-center justify-between">
                ...
                >
                  ...
                  <span className="text-sm text-gray-500 ml-2">{formatCurrency(level.price)}</span>
                </button>
                <button
      ...
      {deleteTarget && (
        <ConfirmDialog
          message={`"${deleteTarget.name}" 레벨을 삭제하시겠습니까?`}
