...
import { formatCurrency, getDateKey } from '../../utils/format'
...
export function ChoreoLessonForm({ date, editLesson, onSubmit, onCancel }: ChoreoLessonFormProps) {
  ...
  const [durationHours, setDurationHours] = useState(String(editLesson?.durationHours ?? 1))
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-1">타임 수</label>
        <input
          type="number"
          inputMode="decimal"
          value={durationHours}
